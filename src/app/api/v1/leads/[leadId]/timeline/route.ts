...
import { timelineFilterSchema } from "@/modules/lead-timeline/validators/timeline.validator";
...
export async function GET(
  ...
  try {
    ...
    const query = timelineFilterSchema.parse({ ...searchParams, lead_id: leadId });

    const service = new TimelineService(supabase);
    const { data, count } = await service.getTimeline(query);
