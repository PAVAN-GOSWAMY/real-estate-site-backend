...
import { releaseInventorySchema } from "@/modules/property-inventory/validators/property-inventory.validator";
...
export async function POST(
  ...
  try {
    ...
    const dto = releaseInventorySchema.parse(body);

    const service = new PropertyInventoryService(supabase);
    const state = await service.releaseInventory(id, dto);

    const mappedData = PropertyInventoryMapper.toStateResponse(state);
    return ApiResponse.success(mappedData, "Inventory released successfully");
  } catch (error: any) {
