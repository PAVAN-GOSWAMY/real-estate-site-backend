...
import { blockInventorySchema } from "@/modules/property-inventory/validators/property-inventory.validator";
...
export async function POST(
  ...
  try {
    ...
    PermissionHelper.requirePermission(session, Permissions.PROPERTIES.INVENTORY.BLOCK);

    const body = await req.json();
    const dto = blockInventorySchema.parse(body);

    const service = new PropertyInventoryService(supabase);
    const state = await service.blockInventory(id, dto);

    const mappedData = PropertyInventoryMapper.toStateResponse(state);
    return ApiResponse.success(mappedData, "Inventory blocked successfully");
  } catch (error: any) {
