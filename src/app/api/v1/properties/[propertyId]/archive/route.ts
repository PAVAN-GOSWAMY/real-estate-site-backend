...
export async function POST(
  ...
  try {
    ...
    PermissionHelper.requirePermission(session, Permissions.PROPERTIES.ARCHIVE);

    const body = await req.json().catch(() => ({}));
    const dto = changePublicationStatusSchema.parse(body);

    const service = new PropertyPublicationService(supabase);
    const transition = await service.archiveProperty(id, dto);

    const mappedData = PropertyPublicationMapper.toResponse(transition);
    return ApiResponse.success(mappedData, "Property archived successfully");
  } catch (error: any) {
