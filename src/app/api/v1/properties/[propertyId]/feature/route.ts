...
import { featurePropertySchema } from "@/modules/property-recommendations/validators/property-recommendations.validator";
...
export async function POST(
  ...
  try {
    ...
    const dto = featurePropertySchema.parse(body);

    const service = new PropertyRecommendationService(supabase);
    const rec = await service.featureProperty(id, dto);

    const mappedData = PropertyRecommendationMapper.toResponse(rec);
    return ApiResponse.success(mappedData, "Property featured successfully");
  } catch (error: any) {
