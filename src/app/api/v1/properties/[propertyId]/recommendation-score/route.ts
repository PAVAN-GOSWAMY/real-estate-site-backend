...
import { updateScoreSchema } from "@/modules/property-recommendations/validators/property-recommendations.validator";
...
export async function PUT(
  ...
  try {
    ...
    const dto = updateScoreSchema.parse(body);

    const service = new PropertyRecommendationService(supabase);
    const rec = await service.updateScore(id, dto);

    const mappedData = PropertyRecommendationMapper.toResponse(rec);
    return ApiResponse.success(mappedData, "Property recommendation score updated successfully");
  } catch (error: any) {
