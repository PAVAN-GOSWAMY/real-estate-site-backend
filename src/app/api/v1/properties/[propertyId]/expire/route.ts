...
export async function POST(
  ...
  try {
    ...
    // Note: Expiration might often be called by a system worker or cron,
    // which would use a service role key. If called by user, requires UNPUBLISH perm.
    const authHelper = new AuthHelper(supabase);
    const session = await authHelper.getSession();
    PermissionHelper.requirePermission(session, Permissions.PROPERTIES.UNPUBLISH);

    const body = await req.json().catch(() => ({}));
    const dto = changePublicationStatusSchema.parse(body);

    const service = new PropertyPublicationService(supabase);
    const transition = await service.expireProperty(id, dto);

    const mappedData = PropertyPublicationMapper.toResponse(transition);
    return ApiResponse.success(mappedData, "Property expired successfully");
  } catch (error: any) {
