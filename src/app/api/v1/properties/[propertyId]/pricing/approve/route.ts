...
export async function POST(
  ...
  try {
    ...
    // Expect { revision_id: string, notes?: string } in body
    ...
    const revision = await service.approveRevision(revisionId, id, dto);

    const mappedData = PropertyPricingMapper.toResponse(revision);
    return ApiResponse.success(mappedData, "Pricing revision approved successfully");
  } catch (error: any) {
