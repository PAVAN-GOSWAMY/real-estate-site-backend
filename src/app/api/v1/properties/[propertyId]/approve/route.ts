...
export async function POST(
  ...
  try {
    ...
    PermissionHelper.requirePermission(session, Permissions.PROPERTIES.APPROVE);

    const body = await req.json().catch(() => ({}));
    const dto = changePublicationStatusSchema.parse(body);

    const service = new PropertyPublicationService(supabase);
    const transition = await service.approveProperty(id, dto);

    const mappedData = PropertyPublicationMapper.toResponse(transition);
    return ApiResponse.success(mappedData, "Property approved successfully");
  } catch (error: any) {
