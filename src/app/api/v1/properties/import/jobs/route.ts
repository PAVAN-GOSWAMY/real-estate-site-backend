...
import { getJobsQuerySchema } from "@/modules/bulk-operations/dto/bulk.dto";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const query = getJobsQuerySchema.parse(searchParams);

    const service = new ImportService(supabase);
    const { data, count } = await service.getJobs(query);
