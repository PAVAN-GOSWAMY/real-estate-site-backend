...
import { rescheduleFollowUpSchema } from "@/modules/lead-follow-ups/validators/followup.validator";
...
export async function POST(
  ...
  try {
    ...
    const dto = rescheduleFollowUpSchema.parse(body);

    const service = new FollowUpService(supabase);
    const followup = await service.rescheduleFollowUp(id, dto);

    return ApiResponse.success(followup, "Follow-up rescheduled successfully");
  } catch (error: any) {
