...
import { followUpFilterSchema } from "@/modules/lead-follow-ups/validators/followup.validator";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const query = followUpFilterSchema.parse(searchParams);

    // If regular user, only see their own
    // This could be a policy enforced at DB, but implemented here for explicit RBAC:
    // if (session.role !== 'MANAGER') query.assigned_to = session.userId;
