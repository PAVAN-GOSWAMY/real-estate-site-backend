...
import { checkInSchema } from "@/modules/site-visits/validators/visit.validator";
...
export async function POST(
  ...
  try {
    ...
    PermissionHelper.requirePermission(session, Permissions.SITE_VISITS.CHECK_IN);

    const body = await req.json();
    const dto = checkInSchema.parse(body);

    const service = new VisitService(supabase);
    const result = await service.checkIn(id, dto, session.userId);
