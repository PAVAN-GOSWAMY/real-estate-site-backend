...
import { rescheduleSchema } from "@/modules/site-visits/validators/visit.validator";
...
export async function POST(
  ...
  try {
    ...
    const dto = rescheduleSchema.parse(body);

    const service = new VisitService(supabase);
    const result = await service.reschedule(id, dto, session.userId);

    return ApiResponse.success(result, "Visit rescheduled");
  } catch (error: any) {
